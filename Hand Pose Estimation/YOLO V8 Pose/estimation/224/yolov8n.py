...
model_path = dir + "\\models\\yolo 224\\yolov8n\\best.onnx"
# model_path = dir + "\\models\\yolo 224\\yolov8n\\best_float32.tflite"
...
out_mp4 = cv2.VideoWriter("YOLO-v8n_eval.mp4",cv2.VideoWriter_fourcc('m', 'p', '4', 'v'),30,(frame_width,frame_height))

while True:
    success,img = cap.read()
    if success:
        results = tflite_model.track(img,persist=True,imgsz=224,iou=0.5, save=True)
    ...
    #                     img = cv2.circle(img, (int(indexxy[0]), int(indexxy[1])), 12, (0, 255, 0), -10) ##index
        ...
        cv2.imshow("YOLO V8N",annotated_frame)
