...
model_path = dir + "\\models\\yolo 224\\yolov8s\\best.onnx"
# model_path = dir + "\\models\\yolo 224\\yolov8s\\best_float32.tflite"
...
while True:
    ...
    if success:
        results = tflite_model.track(img,imgsz=224,persist=True)
    ...
    #                     img = cv2.circle(img, (int(indexxy[0]), int(indexxy[1])), 12, (0, 255, 0), -10) ##index
        ...
        cv2.imshow("YOLO V8S",annotated_frame)
